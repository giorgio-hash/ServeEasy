...
\subsection{x4 numero ordine effettuato: AAAAAAAAAAAAAAAAAAAA}
...
\begin{algorithm}[h]
	\begin{algorithmic}[h!]
		...
		\medskip
		\If{$\text{numOrdineEffettuato} < \text{maxNumOrdineEffettuato}$}
		\State $x4 \gets \frac{\text{numOrdineEffettuato} - 1}{\text{maxNumOrdineEffettuato} - 1}$ \Comment{Normalizzazione}
		\Else
...
\begin{algorithm}[h]
	\begin{algorithmic}[h!]
		...
		\If{$\text{codaPostazione.isFull()} = \text{false} \land \text{numeroOrdiniTotaliInCucina()} < C$}
